...
export function IllustrationPanel({ originalImage, result, phase, mediaStatus }: IllustrationPanelProps) {
  if (!originalImage && !result) return null;

  const isGenerating = phase === "narrating" || phase === "generating";
  const imgFailed = phase === "done" && !result?.illustration_url && mediaStatus.imgState === "failed";
      ...
      {originalImage && (
        <div className="space-y-2">
          ...
          <div className="relative overflow-hidden rounded-xl aspect-square">
            <img src={originalImage} alt="Uploaded photograph" className="w-full h-full object-cover" style={{ filter: "sepia(0.15) contrast(1.05)" }} />
          </div>
        </div>
      )}
      ...
      <div className="space-y-2">
        ...
        <div className="relative overflow-hidden rounded-xl aspect-square bg-stone-100">
          {result?.illustration_url ? (
            <img src={result.illustration_url} alt="AI illustration" className="w-full h-full object-cover animate-fade-in" />
          ) : imgFailed ? (
            <div className="w-full h-full flex flex-col items-center justify-center gap-2 p-4">
              <XCircle className="w-8 h-8 text-red-300" />
              <p className="text-xs font-body text-stone-400 text-center">Illustration unavailable</p>
              {mediaStatus.imgError && <p className="text-xs font-body text-red-400 text-center truncate w-full" title={mediaStatus.imgError}>{mediaStatus.imgError}</p>}
            </div>
          ) : (
              ...
              {isGenerating && (
                <div className="relative z-10 flex flex-col items-center gap-2">
                  ...
                  <p className="text-xs font-body text-stone-400">
                    {mediaStatus.imgState === "retrying" ? `Retrying... (${mediaStatus.imgRetry + 1}/3)` : "Painting..."}
                  </p>
                </div>
              )}
            </div>
